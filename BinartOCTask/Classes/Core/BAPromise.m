...
+ (BAPromise *)resolved:(id)result {
    BADeferred *deferred = [[BADeferred alloc] init];
...
+ (BAPromise *)chain:(promise_returning_arg_block)firstBlock, ... NS_REQUIRES_NIL_TERMINATION {
    ...
    BADeferred *chainedResult = [BADeferred deferred];
    ...
    resolved_block resolveBlock = ^(id result) {
        if (eachResultArray.count == 0) {
            ...
            BAPromise *nextPromise = nextBlock(result);
...
+ (BAPromise *)or:(NSArray *)promises {
    ...
    for (BAPromise *promise in promises) {
...
- (BAPromise *)on:(dispatch_queue_t)queue {
    BADeferred *deferred = [[BADeferred alloc] initWithQueue:queue];
...
- (BAPromise *)timeout:(NSTimeInterval)interval leeway:(NSTimeInterval)leeway {
    BADeferred *toTimeout = [[BADeferred alloc] initWithQueue:_queue];
...
- (BAPromise *)transform:(transform_block)block {
    BADeferred *transformed = [BADeferred deferred];
...
- (void)chainTo:(BADeferred *)deferred {
    [self when:^(id result){
        [deferred resolve:result];
    } failed:^(NSError *error){
        [deferred reject:error];
